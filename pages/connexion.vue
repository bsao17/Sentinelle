<template>
  <AuthFormLayout>
    <div class="auth-header">
      <h1>Connexion</h1>
      <p class="auth-subtitle">Accédez à votre tableau de bord Sentinelle</p>
    </div>
    <form @submit.prevent="handleSubmit" class="form-grid">
      ...
      <label>
        Mot de passe
        <input type="password" v-model="password" required placeholder="Votre mot de passe" />
      </label>
      <p v-if="error" class="error">{{ error }}</p>
      <button type="submit" class="btn-submit" :disabled="loading">{{ loading ? 'Connexion...' : 'Se connecter' }}</button>
    </form>
    <div class="auth-links">
      <p><NuxtLink to="/mot-de-passe-oublie">Mot de passe oublié ?</NuxtLink></p>
      <p>Nouveau sur Sentinelle ? <NuxtLink to="/inscription">Créer un compte</NuxtLink></p>
    </div>
  </AuthFormLayout>
</template>
...
.auth-links p { font-size: var(--font-size-sm); color: var(--text-secondary); margin: var(--spacing-sm) 0; }
</style>
